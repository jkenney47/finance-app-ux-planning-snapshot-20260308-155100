...
import { getAuthenticatedUserId } from "@/supabase/functions/plaidExchangeToken/auth";

describe("plaidExchangeToken auth regression", () => {
  ...
  test("entrypoint is wired to auth helper", () => {
    const source = fs.readFileSync(
      path.join(
        process.cwd(),
        "supabase/functions/plaidExchangeToken/index.ts",
      ),
      "utf8",
    );
