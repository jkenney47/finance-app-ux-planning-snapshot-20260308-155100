...
import { getAuthenticatedUserId } from "@/supabase/functions/plaidAccounts/auth";

describe("plaidAccounts auth regression", () => {
  ...
  test("entrypoint is wired to auth helper", () => {
    const source = fs.readFileSync(
      path.join(process.cwd(), "supabase/functions/plaidAccounts/index.ts"),
      "utf8",
    );
